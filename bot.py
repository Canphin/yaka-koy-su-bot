...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .btn-success{background:#4CAF50}.btn-orange{background:#FF9800}.btn-danger{background:#f44336}
        ...
        .badge-odendi{background:#E8F5E9;color:#2E7D32}.badge-odenmedi{background:#FFF3E0;color:#E65100}
...
<body>
    ...
    <script>
        let tumAboneler=[],tumFaturalar=[],seciliAbone=null,sifre=location.search.split('sifre=')[1]?.split('&')[0]||'';
        function modalAc(id){document.getElementById(id).style.display='flex'}
        function modalKapat(id){document.getElementById(id).style.display='none'}
        window.onclick=function(e){if(e.target.classList.contains('modal'))e.target.style.display='none'}
        function sekmeDegistir(s){document.querySelectorAll('.tab-btn').forEach(b=>b.classList.remove('active'));event.target.classList.add('active');document.getElementById('aboneSekmesi').style.display=s=='aboneler'?'block':'none';document.getElementById('faturaSekmesi').style.display=s=='faturalar'?'block':'none';if(s=='faturalar')faturalariYukle()}
        async function aboneleriYukle(){const r=await fetch('/api/aboneler?sifre='+sifre);tumAboneler=await r.json();let h='';tumAboneler.forEach(a=>{h+=`<tr><td>#${a[0]}</td><td>${a[1]}</td><td>${a[4]}</td><td>${a[7]}</td><td>${a[8]} m³</td><td><button class="btn btn-sm" onclick="endeksAc('${a[0]}','${a[1]}',${a[8]})">✏️</button><button class="btn btn-sm btn-orange" onclick="aboneDuzenle('${a[0]}')">📝</button><button class="btn btn-sm btn-danger" onclick="aboneSil('${a[0]}')">🗑️</button></td></tr>`});document.getElementById('aboneTablo').innerHTML=h||'<tr><td colspan="6">Abone yok</td></tr>';istatistikleriYukle()}
        function aboneFiltrele(){const q=document.getElementById('aboneArama').value.toLowerCase();const f=tumAboneler.filter(a=>a[0].includes(q)||a[1].toLowerCase().includes(q));let h='';f.forEach(a=>{h+=`<tr><td>#${a[0]}</td><td>${a[1]}</td><td>${a[4]}</td><td>${a[7]}</td><td>${a[8]} m³</td><td><button class="btn btn-sm" onclick="endeksAc('${a[0]}','${a[1]}',${a[8]})">✏️</button></td></tr>`});document.getElementById('aboneTablo').innerHTML=h||'<tr><td colspan="6">Yok</td></tr>'}
...
def api_faturalar():
    if not api_yetkili(request): return jsonify({'hata':'Yetkisiz'}), 403
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute("SELECT * FROM faturalar ORDER BY fatura_tarihi DESC LIMIT 100"); data = c.fetchall()
    conn.close(); return jsonify(data)
...
def api_istatistik():
    ...
    c.execute("SELECT COUNT(*) FROM faturalar WHERE date(fatura_tarihi)=?", (bugun,)); bugun_oku = c.fetchone()[0]
    c.execute("SELECT COALESCE(SUM(toplam_tutar),0) FROM faturalar WHERE durum='odenmedi'"); borc = c.fetchone()[0]
    c.execute("SELECT fatura_tarihi FROM faturalar ORDER BY fatura_tarihi DESC LIMIT 1"); son = c.fetchone()
    conn.close(); return jsonify({'abone':abone,'bugun':bugun_oku,'borc':borc,'son':son[0][:16] if son else '-'})
...
def api_toplu_odeme():
    if not api_yetkili(request): return jsonify({'hata':'Yetkisiz'}), 403
    data = request.get_json(); abone_no, tutar = data['abone_no'], data['tutar']
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute("SELECT fatura_no,toplam_tutar FROM faturalar WHERE abone_no=? AND durum='odenmedi' ORDER BY fatura_tarihi", (abone_no,))
    faturalar = c.fetchall(); odenen = 0; kalan = tutar
    for f in faturalar:
        if kalan >= f[1]: c.execute("UPDATE faturalar SET durum='odendi' WHERE fatura_no=?", (f[0],)); kalan -= f[1]; odenen += 1
    conn.commit(); conn.close(); return jsonify({'basarili':True,'odenen':odenen})
...
async def abone_sorgu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT * FROM faturalar WHERE abone_no=? ORDER BY fatura_tarihi DESC LIMIT 3", (context.args[0],)); faturalar = c.fetchall(); conn.close()
    m = f"📋 #{abone[0]} {abone[1]}\n📍 {abone[4]}\n🔢 {abone[8]} ton\n"
    for f in faturalar: m += f"\n📅 {f[9][:10]} | {f[4]} ton | {f[8]} TL | {f[12]}"
...
async def rapor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not yetkili_mi(update): await update.message.reply_text("⛔ Yetkisiz!"); return
    bugun = datetime.now().strftime('%Y-%m-%d'); conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute("SELECT COUNT(*), COALESCE(SUM(tuketim_ton),0), COALESCE(SUM(toplam_tutar),0) FROM faturalar WHERE date(fatura_tarihi)=?", (bugun,)); o = c.fetchone(); conn.close()
